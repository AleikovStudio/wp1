                ...
                $productItemHtml = preg_replace_callback(
                    ...
                    function ($product_variations) {
                        ...
                        if ($product_type == 'variable') {
                            $attribute_parameters = $product->get_variation_attributes();
                            if (count($attribute_parameters) > 0) {
                                foreach ($attribute_parameters as $name => $attribute_parameter) {
                                    ...
                                    if (isset($productAttribute['id']) && $productAttribute['id'] > 0) {
                                        $attribute = wc_get_attribute($productAttribute['id']);
                                        $variation_options = $attribute_parameter;
                                        $variation_title = $attribute->name ? $attribute->name : $attribute->slug;
                                    }
                                    ...
                                    foreach ($variation_options as $index => $option) {
                                        ...
                                        $doubleOptionHtml = preg_replace('/value=[\'"][\s\S]+?[\'"]/', 'value="' . $option . '"', $doubleOptionHtml);
                                        $doubleOptionHtml = preg_replace('/<!--product_variation_option_content-->([\s\S]+?)<!--\/product_variation_option_content-->/', $option, $doubleOptionHtml);
