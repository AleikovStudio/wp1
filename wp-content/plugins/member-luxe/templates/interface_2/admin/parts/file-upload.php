    ...
    <div class="wpm-fileupload" data-id="<?php echo $id; ?>" data-name="<?php echo $name; ?>">
       ...
       <div class="fileupload-buttons">
            <label class="wpm_jqhfu-file-container">
				...
				<span class="wpm-button wpm-homework-edit-popup-addfile-button">Выбрать файлы</span>
            </label>
    ...
    <tr class="template-upload wpm_jqhfu-fade">
        ...
        <td>
            {% if (!i && !o.options.autoUpload) { %}
                <button class="start wpm-button wpm-homework-edit-popup-upload-button" disabled>Загрузить</button>
            {% } %}
            {% if (!i) { %}
                <button class="cancel wpm-button wpm-homework-edit-popup-cancel-button">Отмена</button>
            {% } %}
        </td>
    </tr>
{% } %}
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
    <tr class="template-download wpm_jqhfu-fade {% if (file.error) { %} wpm_jqhfu-file-error {% } %} ">
        ...
        <td>
            <button class="delete wpm-button wpm-homework-edit-popup-delete-button" data-type="{%=file.deleteType%}" data-url="{%=file.deleteUrl%}&action=load_ajax_function"{% if (file.deleteWithCredentials) { %} data-xhr-fields='{"withCredentials":true}'{% } %}>Удалить</button>
        </td>
    </tr>
{% } %}
</script>
