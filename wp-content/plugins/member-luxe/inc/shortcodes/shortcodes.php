...
class WPMShortcodes{
    ...
    function wpm_gallery_shortcode($attr)
    {
        ...
        foreach ($attachments as $image) {
            $full_image = wpm_remove_protocol(wp_get_attachment_image_src($image->ID, 'full', 0));
            $slide_image = wpm_remove_protocol(wp_get_attachment_image_src($image->ID, 'wpm-slider', 0));

            $output .= sprintf($tpl, $full_image[0], self::$_galleryCounter, self::$_galleryCounter, $slide_image[0]);
        }
...
<script type="IN/Share" data-counter="top" data-onsuccess="linkedin_share_click"></script></div>';
    ...
    function wpm_countdown_shortcode($atts)
    {
        ...
        // create timer image
        if(!empty($image)){
            $c_image = '<img class="c-image" src="//static.wppage.ru/wppage/i/timer/'.$image.'.png">';
        }else{
            $c_image = '';
        }

        // use image as background for adaptive countdown
        $timer_backgound_image = '<img class="timer-background-image" src="//static.wppage.ru/wppage/i/countdown/skins/'.$skin.'/color-'.$bg_color.'/bg.png">';
    ...
    {
        if ($options[key]) {
            return '<iframe src="//spreadsheets.google.com/embeddedform?formkey=' . $options[key] . '" width="' . $options[width] . '" height="' . $options[height] . '" frameborder="0" marginheight="0" marginwidth="0">Загружается...</iframe>';
        } elseif($options[url]) {
            ...
            if(!$google_form_vars[formkey]){
                ...
                return '<iframe src="//spreadsheets.google.com/embeddedform?formkey=' . $google_form_vars[formkey] . '" width="' . $options[width] . '" height="' . $options[height] . '" frameborder="0" marginheight="0" marginwidth="0">Загружается...</iframe>';
            }
