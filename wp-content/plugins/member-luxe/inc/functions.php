...
function add_wpm_page_metabox()
{
    ...
    if(wpm_is_interface_2_0()) {
        add_meta_box('wpm_page_content_types_metabox', __('Тип контента', 'mbl'), 'wpm_page_content_types_metabox', 'wpm-page', 'side', 'high');
    }
}
...
function wpm_admin_menu()
{
    ...
    add_submenu_page('edit.php?post_type=wpm-page', __('Статистика входов', 'wpm'), __('Статистика входов', 'wpm'), 'manage_options', 'wpm-login-stats', 'wpm_stats_admin');
...
Приятной работы!'
            ...
            'response_review'          => array(
                'title'   => 'Комментарий к Вашему ответу',
                'content' => 'Материал: 
[material_name]
...
Перейти на страницу материала: [material_url]'
            ...
            'response_status'          => array(
                'title'   => 'Результат выполнения домашнего задания',
                'content' => 'Материал: 
[material_name]
...
Перейти на страницу материала: [material_url]'
            ...
            'response_admin'          => array(
                'title'   => 'Ответ на домашнее задание',
                'content' => 'Материал: 
[material_name]
...
//            }
            ...
            if(wpm_is_interface_2_0()) {
                ...
                wpm_enqueue_style('wpm-bootstrap', plugins_url('/member-luxe/2_0/bootstrap/css/bootstrap-admin.css'));
                wpm_enqueue_script('wpm-bootstrap', plugins_url('/member-luxe/2_0/bootstrap/js/bootstrap.js'));
                wpm_enqueue_style('wpm-summernote', plugins_url('/member-luxe/js/summernote/summernote.css'));
                wpm_enqueue_script('wpm-summernote', plugins_url('/member-luxe/js/summernote/summernote.min.js'));
...
function wpm_enqueue_styles()
{
    ...
    if(wpm_is_interface_2_0()) {
        wpm_enqueue_style('wpm-bootstrap', plugins_url('/member-luxe/2_0/bootstrap/css/bootstrap.css'));
        wpm_enqueue_style('wpm-app', plugins_url('/member-luxe/2_0/css/app.css'));
        ...
        wpm_enqueue_style('wpm-summernote', plugins_url('/member-luxe/js/summernote/summernote.css'));
        wpm_enqueue_style('wpm-plyr', plugins_url('/member-luxe/js/plyr/plyr.css'));
        wpm_enqueue_style('wpm-fancybox', plugins_url('/member-luxe/2_0/fancybox/jquery.fancybox.min.css'));
        //wpm_enqueue_style('wpm-countdown', plugins_url('/member-luxe/js/countdown/jquery.countdown.css')); // replaced by 2_0/scss/partials/_countdown.scss
    } else {
...
function wpm_enqueue_scripts()
{
    ...
    if(wpm_is_interface_2_0()) {
        wpm_enqueue_script('wpm-jquery', plugins_url('/member-luxe/2_0/js/jquery/jquery-2.2.4.min.js'));
        wpm_enqueue_script('wpm-jquery-form', plugins_url('/member-luxe/js/jquery/jquery.form.js'));
        wpm_enqueue_script('wpm-jquery-ui', plugins_url('/member-luxe/2_0/js/jquery-ui/jquery-ui.min.js'));
        wpm_enqueue_script('wpm-jquery-ui-touch-punch', plugins_url('/member-luxe/2_0/js/jquery-ui/jquery.ui.touch-punch.min.js'));
        wpm_enqueue_script('wpm-bootstrap', plugins_url('/member-luxe/2_0/bootstrap/js/bootstrap.js'));
        wpm_enqueue_script('wpm-clamp', plugins_url('/member-luxe/2_0/js/jquery.shave.min.js'));
        wpm_enqueue_script('wpm-owl', plugins_url('/member-luxe/js/owl.carousel/owl.carousel.min.js'));
        wpm_enqueue_script('wpm-summernote', plugins_url('/member-luxe/js/summernote/summernote.min.js'));
        wpm_enqueue_script('wpm-summernote-lang', plugins_url('/member-luxe/js/summernote/lang/summernote-ru-RU.js'));
        wpm_enqueue_script('wpm-jquery-fancybox', plugins_url('/member-luxe/2_0/fancybox/jquery.fancybox.min.js'));
        ...
        wpm_enqueue_script('wpm-jquery-ui-cookie', plugins_url('/member-luxe/js/miscellaneous/jquery.cookies.2.2.0.min.js'));
        ...
        wpm_enqueue_script('wpm-jquery-countdown-ru', plugins_url('/member-luxe/js/countdown/jquery.countdown-ru.js'));

        wpm_enqueue_script('wpm-app', plugins_url('/member-luxe/2_0/js/app.js'));
    } else {
...
function wpm_get_template($template)
{
    ...
    } elseif (is_tax('wpm-category') && !is_search()) {
...
function wpm_get_all_user_accesible_levels($user_id)
{
    ...
    return count($term_ids) ? get_terms('wpm-levels', array('fields'=>'ids', 'include' => $term_ids)) : array();
}
...
function wpm_get_excluded_categories($levelId = null)
{
    ...
    if (!in_array('administrator', $current_user->roles) || $levelId !== null) {
        ...
        $user_level_ids = ($current_user->ID && count($user_level_ids))
            ? get_terms('wpm-levels', array('fields'=>'ids', 'include' => $user_level_ids))
            : array();
...
{
    __wpm_array_set($array, $key, $value);

    return $array;
}

function __wpm_array_set(&$array, $key, $value)
{
    if (is_null($key)) {
        return $array = $value;
    }
...
function wpm_material_link($category = null, $post = null)
{
    ...
    if ($isCategory && $post) {
        if (get_option('permalink_structure') != '') {
            $link = sprintf("wpm/%s/%s/", $category->slug, $post->post_name);
        } else {
...
function wpm_rewrite_request($vars)
{
    if (wpm_array_get($vars, 'wpm-page') && !wpm_array_get($vars, 'wpm-category')) {
        $post = get_page_by_path(wpm_array_get($vars, 'wpm-page'), OBJECT, 'wpm-page');
...
function wpm_get_avatar($userId = null, $size = 150, $id = null, $fullTag = true, $withGravatar = true, $isComment = false)
{
//    global $current_user;
    ...
    if($withGravatar) {
        $defaultFile = $isComment ? 'default-comment-avatar.svg' : 'default-profile-image.svg';
        $default = plugins_url('/member-luxe/2_0/images/' . $defaultFile);
        ...
        if (!preg_match("|200|", wpm_array_get($headers, 0, ''))) {
            $gravatar = sprintf('<img src="%s" class="avatar avatar-%s photo" id="%s">', $default, $size, $id);
        }
    }
...
{
    $avatar = wpm_get_avatar($userId, $size, null, true, true, $isComment);
...
function mbl_admin_sample_permalink($permalink, $id)
{
    ...
    if($post->post_type == 'wpm-page') {
        ...
        if (count($terms)) {
            if(get_option('permalink_structure') != '') {
                $link = sprintf("wpm/%s/%s/", $terms[0]->slug, '%pagename%');
            } else {
                $link = "?wpm-page=%pagename%&wpm-category={$terms[0]->slug}";
            }
            $permalink[0] = home_url($link);
        }
    }
...
add_filter('post_type_link', 'mbl_admin_post_link',10,2);

function mbl_admin_post_link($permalink, $post)
{
